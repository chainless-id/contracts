...
export class AASigner extends Signer {
  ...
  async _deploymentTransaction(): Promise<BytesLike> {
    ...
    return new SimpleWallet__factory().getDeployTransaction(
    ...
    const waitPromise = new Promise<TransactionReceipt>((resolve, reject) => {
      let listener = async function (this: any, ...param: any) {
        ...
        if (event.args.requestId != requestId) {
          console.log(
            `== event with wrong requestId: sender/nonce: event.${event.args.sender}@${event.args.nonce}!= userOp.${
              userOp.sender
        ...
        if (!event.args!.success) {
          ...
          if (revertReasonEvents[0]) {
            ...
            reject("UserOp failed with reason: " + revertReasonEvents[0].args.revertReason);
            return;
          }
        }
      ...
      setTimeout(
        () =>
          entryPoint.queryFilter(entryPoint.filters.UserOperationEvent(requestId)).then((query) => {
            if (query.length > 0) {
              listener(query[0]);
            }
          }),
  ...
  async _createUserOperation(transaction: Deferrable<TransactionRequest>): Promise<UserOperation> {
    ...
    const execFromEntryPoint = await this._wallet!.populateTransaction.execFromEntryPoint(
      tx.to!,
      tx.value ?? 0,
      tx.data!
    );
