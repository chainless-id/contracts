...
export async function checkForBannedOps(txHash: string, checkPaymaster: boolean) {
  const debugTx = async (hash: string) =>
    ethers.provider.send("debug_traceTransaction", [
      hash,
      {
        disableMemory: true,
        disableStorage: true,
...
export async function deployEntryPoint(
  ...
  const ctrParams = defaultAbiCoder.encode(
    ...
    [Create2Factory.contractAddress, paymasterStake, unstakeDelaySecs, "0x0000000000000000000000000000000000000000"]
  );
