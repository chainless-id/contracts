...
describe("Batch gas testing", function () {
    ...
    describe("test batches maxCount=" + maxCount, () => {
      ...
      it("batch of create", async () => {
        ...
        while (++count) {
          ...
          const wallet1 = await entryPoint.getSenderAddress(
            WalletConstructor(entryPoint.address, walletOwner1.address),
            0
          );
      ...
      it("batch of expensive ops", async function () {
        ...
        walletExecFromEntryPoint_waster = await wallet.populateTransaction.execFromEntryPoint(
          counter.address,
          0,
          waster.data!
        );
  ...
  async function call_handleOps_and_stats(title: string, ops: UserOperation[], count: number) {
    ...
    const entireTxEncoded = toBuffer(
      await entryPoint.populateTransaction.handleOps(ops, beneficiaryAddress).then((tx) => tx.data)
    );
    ...
    const totalEventsGasCost = parseInt(
      events1
        .map((x) => x.args!.actualGasCost)
        .reduce((sum, x) => sum.add(x))
        .toString()
    );

    const senderPaid = parseInt(
      senderPrebalance.sub(await ethers.provider.getBalance(await sender.getAddress())).toString()
    );
