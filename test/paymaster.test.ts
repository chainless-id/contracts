  ...
  describe("using TokenPaymaster (account pays in paymaster tokens)", () => {
    ...
    describe("#handleOps", () => {
      ...
      before(async () => {
        ...
        calldata = await wallet.populateTransaction
          .execFromEntryPoint(wallet.address, 0, updateEntryPoint)
          .then((tx) => tx.data!);
      });
    ...
    describe("create account", () => {
      ...
      it("should succeed to create account with tokens", async () => {
        const preAddr = await entryPoint.getSenderAddress(
          WalletConstructor(entryPoint.address, walletOwner.address),
          0
        );
      ...
      it("account should pay for its creation (in tst)", async function () {
        ...
        const walletAddr = await entryPoint.getSenderAddress(
          WalletConstructor(entryPoint.address, walletOwner.address),
          0
        );
      ...
      it("batched request should each pay for its share", async () => {
        ...
        const execFromSingleton = wallet.interface.encodeFunctionData("execFromEntryPoint", [
          testCounter.address,
          0,
          justEmit,
        ]);
        ...
        for (let i = 0; i < 4; i++) {
          const aWallet = await new SimpleWallet__factory(ethersSigner).deploy(
            entryPoint.address,
            await walletOwner.getAddress(),
      ...
      describe("grief attempt", () => {
        ...
        before(async () => {
          wallet2 = await new SimpleWallet__factory(ethersSigner).deploy(
            entryPoint.address,
            await walletOwner.getAddress(),
            "0x0000000000000000000000000000000000000000"
          );
          ...
          const approveOp = await fillAndSign(
            {
              ...
              callData: wallet2.interface.encodeFunctionData("execFromEntryPoint", [
                paymaster.address,
                0,
                approveCallData,
              ]),
        ...
        it("griefing attempt should cause handleOp to revert", async () => {
          ...
          const execFromEntryPoint = wallet.interface.encodeFunctionData("execFromEntryPoint", [
            paymaster.address,
            0,
            withdrawTokens,
          ]);
          ...
          await expect(entryPoint.handleOps([userOp1, userOp2], beneficiaryAddress)).to.be.revertedWith(
    ...
    describe("withdraw", () => {
      ...
      it("should fail to withdraw before unstake", async () => {
        ...
        await expect(paymaster.withdrawTo(withdrawAddress, amount)).to.revertedWith("must call unstakeDeposit");
      });
