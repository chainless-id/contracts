...
describe("EntryPoint", function () {
  ...
  describe("#simulateValidation", () => {
    ...
    before(async () => {
      wallet1 = await new SimpleWallet__factory(ethersSigner).deploy(
        entryPoint.address,
        await walletOwner1.getAddress(),
        "0x0000000000000000000000000000000000000000"
      );
    });
    ...
    it("should fail if validateUserOp fails", async () => {
      ...
      await expect(entryPointView.callStatic.simulateValidation(op).catch(rethrow())).to.revertedWith(
        "wrong signature"
      );
    });
    ...
    it("should prevent overflows: fail if any numeric value is more than 120 bits", async () => {
      ...
      await expect(entryPointView.callStatic.simulateValidation(op)).to.revertedWith("gas values overflow");
    });
    ...
    it("should fail creation for wrong sender", async () => {
      ...
      await expect(entryPointView.callStatic.simulateValidation(op1).catch(rethrow())).to.revertedWith(
  ...
  describe("without paymaster (account pays in eth)", () => {
    describe("#handleOps", () => {
      ...
      it("#handleOp (single)", async () => {
        ...
        const countBefore = await counter.counters(wallet.address);
        const rcpt = await entryPoint
          .handleOp(op, beneficiaryAddress, {
            gasLimit: 1e7,
          })
          .then((t) => t.wait());
        const countAfter = await counter.counters(wallet.address);
    ...
    describe("batch multiple requests", () => {
      ...
      before("before", async () => {
        counter = await new TestCounter__factory(ethersSigner).deploy();
        const count = await counter.populateTransaction.count();
        walletExecCounterFromEntryPoint = await wallet.populateTransaction.execFromEntryPoint(
          counter.address,
          0,
          count.data!
        );
