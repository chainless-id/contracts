...
import { SimpleWallet, SimpleWallet__factory, TestUtil, TestUtil__factory } from "../typechain";
...
describe("SimpleWallet", function () {
  ...
  it("other account should not be able to call transfer", async () => {
    ...
    await expect(wallet.connect(ethers.provider.getSigner(1)).transfer(accounts[2], ONE_ETH)).to.be.revertedWith(
      "only owner"
    );
  });
  ...
  describe("#validateUserOp", () => {
    ...
    before(async () => {
      ...
      wallet = await new SimpleWallet__factory(await ethers.getSigner(entryPoint)).deploy(
        entryPoint,
        walletOwner.address,
        "0x0000000000000000000000000000000000000000"
      );
